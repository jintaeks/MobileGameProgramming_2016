...
void DrawBoard(HDC hdc_, const KBoard& board_)
{
    ...
    matScale.SetScale(10, 10, 10);
    matProjection.SetProjection(1000.f);

    matTransform = matTrans * matRotY * matRotX * matScale;
    ...
    float xWidth = g_clientRect.right - g_clientRect.left;
    float yHeight = g_clientRect.bottom - g_clientRect.top;
    for (int y = 0; y < BOARD_SIZE_ROW; ++y)
    {
        for (int x = 0; x < BOARD_SIZE_COL; ++x)
        {
            ...
            if (cellValue >= 1)
            {
                matTrans.SetTranslation(128 + x * 32, y * 32, 0);
                matTransform = matTrans;
                poly.SetVertexBuffer();
                poly.Transform(matTransform);
                poly.Projection(matProjection);
                //poly.ApplyViewport(xWidth/2.f, 0);
                poly.Render(hdc_);
                //Rectangle(hdc_, x * 32, y * 32, x * 32 + 32, y * 32 + 32);
            }//if
        }//for
    }//for
}//DrawBoard()
