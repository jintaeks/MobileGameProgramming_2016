...
static void bkp_forward(bkp_network_t *n)
{
   ...
   n->LearningError = n->LearningError / 2.0f;
}
...
int bkp_loadfromfile(bkp_network_t **n, char *fname)
{
   ...
   if (_read(fd, (int *) &(*n)->NumInputs, sizeof(int)) == -1)
      goto errandret;
   if( _read( fd, (int *)&( *n )->NumHidden, sizeof( int ) ) == -1 )
      goto errandret;
   if( _read( fd, (int *)&( *n )->NumOutputs, sizeof( int ) ) == -1 )
      goto errandret;
   if( _read( fd, (int *)&( *n )->NumBias, sizeof( int ) ) == -1 )
      goto errandret;
		
   if( _read( fd, (int *)&( *n )->InputReady, sizeof( int ) ) == -1 )
      goto errandret;
   if( _read( fd, (int *)&( *n )->DesiredOutputReady, sizeof( int ) ) == -1 )
      goto errandret;
   if( _read( fd, (int *)&( *n )->Learned, sizeof( int ) ) == -1 )
      goto errandret;
		
   if( _read( fd, ( *n )->InputVals, ( *n )->NumInputs * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->DesiredOutputVals, ( *n )->NumOutputs * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->IHWeights, ( *n )->NumInputs * ( *n )->NumHidden * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->PrevDeltaIH, ( *n )->NumInputs * ( *n )->NumHidden * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->PrevDeltaHO, ( *n )->NumHidden * ( *n )->NumOutputs * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->PrevDeltaBH, ( *n )->NumBias * ( *n )->NumHidden * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->PrevDeltaBO, ( *n )->NumBias * ( *n )->NumOutputs * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->HiddenVals, ( *n )->NumHidden * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->HiddenBetas, ( *n )->NumHidden * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->HOWeights, ( *n )->NumHidden * ( *n )->NumOutputs * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->BiasVals, ( *n )->NumBias * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->BHWeights, ( *n )->NumBias * ( *n )->NumHidden * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->BOWeights, ( *n )->NumBias * ( *n )->NumOutputs * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->OutputVals, ( *n )->NumOutputs * sizeof( float ) ) == -1 )
      goto errandret;
   if( _read( fd, ( *n )->OutputBetas, ( *n )->NumOutputs * sizeof( float ) ) == -1 )
      goto errandret;
 ...
 *  8. Current input values (InputVals) (NumInputs * sizeof(float))
 *  9. Current desired output values (DesiredOutputVals) (NumOutputs * sizeof(float))
 * 10. Current input-hidden weights (IHWeights) (NumInputs * NumHidden * sizeof(float))
 * 11. Previous input-hidden weight deltas (PrevDeltaIH) (NumInputs * NumHidden * sizeof(float))
 * 12. Previous output-hidden weight deltas (PrevDeltaHO) (NumHidden * NumOutputs * sizeof(float))
 * 13. Previous bias-hidden weight deltas (PrevDeltaBH) (NumBias * NumHidden * sizeof(float))
 * 14. Previous bias-output weight deltas (PrevDeltaBO) (NumBias * NumOutputs * sizeof(float))
 * 15. Current hidden unit values (HiddenVals) (NumHidden * sizeof(float))
 * 16. Current hidden unit beta values (HiddenBetas) (NumHidden * sizeof(float))
 * 17. Current hidden-output weights (HOWeights) (NumHidden * NumOutputs * sizeof(float))
 * 18. Current bias unit values (BiasVals) (NumBias * sizeof(float))
 * 19. Current bias-hidden weights (BHWeights) (NumBias * NumHidden * sizeof(float))
 * 20. Current bias-output weights (BOWeights) (NumBias * NumOutputs * sizeof(float))
 * 21. Current output values (OutputVals) (NumOutputs * sizeof(float))
 * 22. Current output unit betas (OutputBetas) (NumOutputs * sizeof(float))
 *
 * Return Values:
 * int 0: Success
 *    -1: Error, errno is:
...
int bkp_savetofile(bkp_network_t *n, char *fname)
{
   ...
   if (_write(fd, (int *) &n->NumInputs, sizeof(int)) == -1)
      goto cleanupandret;
   if( _write( fd, (int *)&n->NumHidden, sizeof( int ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, (int *)&n->NumOutputs, sizeof( int ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, (int *)&n->NumBias, sizeof( int ) ) == -1 )
      goto cleanupandret;
		
   if( _write( fd, (int *)&n->InputReady, sizeof( int ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, (int *)&n->DesiredOutputReady, sizeof( int ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, (int *)&n->Learned, sizeof( int ) ) == -1 )
      goto cleanupandret;
		
   if( _write( fd, n->InputVals, n->NumInputs * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->DesiredOutputVals, n->NumOutputs * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->IHWeights, n->NumInputs * n->NumHidden * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->PrevDeltaIH, n->NumInputs * n->NumHidden * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->PrevDeltaHO, n->NumHidden * n->NumOutputs * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->PrevDeltaBH, n->NumBias * n->NumHidden * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->PrevDeltaBO, n->NumBias * n->NumOutputs * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->HiddenVals, n->NumHidden * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->HiddenBetas, n->NumHidden * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->HOWeights, n->NumHidden * n->NumOutputs * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->BiasVals, n->NumBias * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->BHWeights, n->NumBias * n->NumHidden * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->BOWeights, n->NumBias * n->NumOutputs * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->OutputVals, n->NumOutputs * sizeof( float ) ) == -1 )
      goto cleanupandret;
   if( _write( fd, n->OutputBetas, n->NumOutputs * sizeof( float ) ) == -1 )
      goto cleanupandret;
