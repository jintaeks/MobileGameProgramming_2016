//////////////////////////////////////////////////////////////////
// File: Graphics2D.cpp
// Desc: implementation of the CGraphics2D class.
...
CGraphics2D::CGraphics2D( void (*fpPutPixel)(int, int, int) )
{
...
// set clipping rectangular region
void CGraphics2D::SetClipRect(int left, int top, int right, int bottom)
{
    m_min_clip_x = left;
    m_max_clip_x = right;
    m_min_clip_y = top;     
    m_max_clip_y = bottom;                  
}//CGraphics2D::SetClipRect(int left, int top, int right, int bottom)

// set PutPixel() callback function
void CGraphics2D::SetPutPixelCallback( void (__cdecl *fp)(int,int,int) )
{
    m_fpPutPixel = fp;
}//CGraphics2D::SetPutPixelCallback(void (__cdecl *fp)(int,int,int))

// OBSOLETE!!!
void CGraphics2D::SetMemSetCallback( void (__cdecl *fp)(int,int) )
{
    m_fpMemSet = fp;
}//CGraphics2D::SetMemSetCallback(void (__cdecl *fp)(int,int,int))

void CGraphics2D::SetCallback( void (__cdecl *fpPutPixel)(int,int,int), 
                          void (__cdecl *fpMemSet)(int,int) /*= NULL*/ )
{
    m_fpPutPixel = fpPutPixel;
    m_fpMemSet   = fpMemSet;
}//CGraphics2D::SetCallback(void (__cdecl *fpPutPixel)(int,int,int), void (__cdecl *fpMemSet)(int,int,int))

#if defined(_DEBUG)
// draw pixel(debug version)
// you must properly set PutPixel() functions address using
//  SetPutPixelCallback() member function.
void CGraphics2D::PutPixel(int x, int y, int value) 
{ 
    ASSERT( m_fpPutPixel != NULL );
    if ( _IsPointIn(x,y) )
        m_fpPutPixel(x, y, value); 
}//CGraphics2D::PutPixel(int x, int y, int value) 

// NOTE: used only in DrawFilledPolygon2D()
//       you must clip test on your callback function
void CGraphics2D::MemSet(int offset, int value) 
{ 
    ASSERT( m_fpMemSet != NULL );
    m_fpMemSet(offset, value); 
}//CGraphics2D::PutPixel(int x, int y, int value) 
#endif //defined(_DEBUG)

// draw line using Bresenham algorithm
// internally call ClipLine() so, line which is outside of clipping rectangle
// will be not drawn.
void CGraphics2D::Bresenham(int xStart, int yStart, int xEnd, int yEnd, int value) 
{
    // clipping line
...
}//CGraphics2D::Bresenham()

// draw rectangle using Bresenham()
void CGraphics2D::Rectangle(int x1, int y1, int x2, int y2, int value)
{
    Bresenham(x1, y1, x2, y1, value);
    Bresenham(x1, y2, x2, y2, value);
    Bresenham(x1, y1, x1, y2, value);
    Bresenham(x2, y1, x2, y2, value);
}//CGraphics2D::Rectangle(int x1, int y1, int x2, int y2, int value)

// return 1 when (x,y) is in clipping region
int CGraphics2D::_IsPointIn(int x, int y)
{
    ...
    if ( y < m_min_clip_y || y > m_max_clip_y ) 
        return 0;
    return 1;
}//CGraphics2D::_IsPointIn()

// plot 8 points of a circle
// internal routine of CircleMidPoint()
void CGraphics2D::_CirclePlotPoints(int xCenter, int yCenter, int x, int y, int value) 
{
    ...
    if ( _IsPointIn(xCenter-y, yCenter-x) )
        PutPixel(xCenter-y, yCenter-x, value);
}//CGraphics2D::_CirclePlotPoints()

// Draw circle using MidPoint algorithm
void CGraphics2D::CircleMidPoint(int xCenter, int yCenter, int radius, int value) 
{
    int x   = 0;
    int y   = radius;
    int p   = 1 - radius;
...
}//CGraphics2D::CircleMidPoint()

// this function clips the sent line using the globally defined clipping
// region
// set Clipping area using SetClipRect()
// clip line (x1,y1)-(x2,y2) using Cohen-Sutherland Clipping Algorithm
int CGraphics2D::ClipLine(int& x1,int& y1,int& x2, int& y2)
{
...
#define CLIP_CODE_SW 0x0005
    ...
    return 1;
}//CGraphics2D::ClipLine(int& x1,int& y1,int& x2, int& y2)

// this function computes the distance from 0,0 to x,y with 3.5% error
int CGraphics2D::FastDistance2D(int x, int y)
{
...
}//CGraphics2D::FastDistance2D()

// this function computes the distance from the origin to x,y,z
float CGraphics2D::FastDistance3D(float fx, float fy, float fz)
{
...
}//CGraphics2D::FastDistance3D()

// draw horizontal line without clipping test(in Release version)
// internally used in DrawTopTri() etc.
void CGraphics2D::_DrawHLine(int x, int y, int width, int value)
{
    int i;

    for (i=0; i<width; ++i)
        PutPixel(x+i, y, value);
}//CGraphics2D::_DrawHLine(int x, int y, int width)

// this function draws a triangle that has a flat top
void CGraphics2D::DrawTopTri(int x1,int y1, int x2,int y2, int x3,int y3, int color )
{
...
}//CGraphics2D::DrawTopTri()

// this function draws a triangle that has a flat bottom
void CGraphics2D::DrawBottomTri(int x1,int y1, int x2,int y2, int x3,int y3, int color )
{
    float   dx_right,    // the dx/dy ratio of the right edge of line
            dx_left,     // the dx/dy ratio of the left edge of line
            xs, xe,      // the starting and ending points of the edges
...
}//CGraphics2D::DrawTriangle2D()

// this function draws a general n-sided polygon 
// NOTE: this functions doesn't test clipping
//       you may add ClipLine() to clip properly
void CGraphics2D::DrawFilledPolygon2D(POLYGON2D_PTR poly)
{
...
}//CGraphics2D::DrawFilledPolygon2D()
